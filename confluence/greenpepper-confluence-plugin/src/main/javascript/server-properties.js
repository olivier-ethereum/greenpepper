	...
	GPProperties.prototype =
	{
		...
		editRegistration:function(id){this.action.editRegistration(this.createParams({id:id, projectName:(_prototype_$('projectName')? $F('projectName') : ''), repositoryName:(_prototype_$('repositoryName')? $F('repositoryName') : ''), readonly:true, editMode:true})); },
		register:function(id){ 
			if($F('repositoryName') === ''){ return; }
			var newProjectName = _prototype_$('newProjectName') ? $F('newProjectName') : 'NA'; 
		...
		getSutsPane:function(id, projectName){ 
			var readonly = _prototype_$('readonly') ? true : false;
