GP.ConfluenceActions=
{
	...
	refreshChildren:function(list, allChildren){ new Ajax.Updater("conf"+list.id, list.params.ctx + "/ajax/greenpepper/RefreshChildren.action", this.query(list, {spaceKey:GP.View.getInnerValue('conf_spaceKey_display'+list.id), pageId:GP.View.getInnerValue('conf_pageId_display'+list.id), forcedSuts:GP.View.getInnerValue('conf_forcedSuts_display'+list.id), showList:GP.View.isVisible('conf_specificationList_display'+list.id), allChildren:allChildren, sortType:_prototype_$('sortType'+list.id).value, reverse:_prototype_$('reverse'+list.id).value})); },
	refreshLabels:function(list){ new Ajax.Updater("conf"+list.id, list.params.ctx + "/ajax/greenpepper/RefreshLabels.action", this.query(list, {spaceKey:GP.View.getInnerValue('conf_spaceKey_display'+list.id), showList:GP.View.isVisible('conf_specificationList_display'+list.id), forcedSuts:GP.View.getInnerValue('conf_forcedSuts_display'+list.id), labels:GP.View.getInnerValue('conf_labels_display'+list.id), sortType:_prototype_$('sortType'+list.id).value, reverse:_prototype_$('reverse'+list.id).value, openInSameWindow:_prototype_$('openInSameWindow'+list.id).value})); },
	updateExecuteChildren: function(specification){ new Ajax.Updater("conf_"+specification.params.bulkUID+"_"+specification.params.executionUID, specification.params.ctx + "/ajax/greenpepper/UpdateExecuteChildren.action", this.query(specification, {doExecuteChildren:_prototype_$('conf_childrenInput').checked})); },
	...
	run: function(specification, implemented){ new Ajax.Updater("conf_results"+specification.id, specification.params.ctx + "/ajax/greenpepper/Run.action", this.query(specification, {implemented:(implemented ? true : false), sutInfo:_prototype_$('conf_sutInfo_display'+specification.id).innerHTML})); },
