...
public class ShadingTest {
    ...
    public void testCommonsCodecsConflicts() throws IOException {
        ...
        String issue24POM = String.join(File.separator, info.getProperty("it.tests.folder"), "test-issue24", "pom.xml");
        String issue24TestFile = String.join(File.separator, info.getProperty("it.tests.folder"), "test-issue24", "src", "test", "specs", "right.html");
        ...
        String line = String.format("java -jar %s -v -p %s %s %s", shadedJar, issue24POM, issue24TestFile,
                String.join(File.separator, cwdFile.getAbsolutePath(), "testCommonsCodecsConflicts.html"));
