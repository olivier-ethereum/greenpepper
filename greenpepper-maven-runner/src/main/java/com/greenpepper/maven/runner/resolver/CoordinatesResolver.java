...
public class CoordinatesResolver implements ProjectFileResolver.Resolver
{
    ...
    private final Pattern coordinatesPattern = Pattern.compile( "(.*):(.*):(.*)" );
    ...
    private void resolveCoordinates(String value)
    {
        String[] parts = value.split( ":" );
        groupId = parts[0];
        artifactId = parts[1];
        version = parts[parts.length - 1]; //version is always the last, in case user specify the full coordinates
    }
