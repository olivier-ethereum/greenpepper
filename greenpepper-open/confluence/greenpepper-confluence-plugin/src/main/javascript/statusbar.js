...
function StatusChart(displayId, imgDir, fontSize, initView) {
    ...
    this.switchView = function () {
        this.view = (this.view == NUM_VIEW ? BAR_VIEW : this.view + 1);
        this.draw();
    };
    this.wasIgnored = function () {
        return this.successCount == 0 && this.failuresCount == 0 && this.errorsCount == 0;
    };
    this.wasSuccessfull = function () {
        return this.successCount != 0 && this.failuresCount == 0 && this.errorsCount == 0;
    };
    this.hasFailed = function () {
        return this.failuresCount != 0 || this.errorsCount != 0;
    };
    ...
    this.addSuccess = function () {
        if (this.success <= this.totalTics) {
            this.success++;
        }
        this.draw();
    };
    this.addFailure = function () {
        if (this.failures <= this.totalTics) {
            this.failures++;
        }
        this.draw();
    };
    ...
    this.drawGifBar = function () {
        ...
        if (limit < 0) {
            this.totalTics = this.success + this.failures;
        }
        for (var i = 0; i < this.success; i++) {
            inHTML = inHTML + "<td style=\"margin: 0px; padding: 0px 1px;\"><img src=\"" + this.imgDir + "chart_success.gif\" class=\"centeredImage\"></td>";
        }
        for (var j = 0; j < this.failures; j++) {
            inHTML = inHTML + "<td style=\"margin: 0px; padding: 0px 1px;\"><img src=\"" + this.imgDir + "chart_failure.gif\" class=\"centeredImage\"></td>";
        }
        for (var z = 0; z < limit; z++) {
            inHTML = inHTML + "<td style=\"margin: 0px; padding: 0px 1px;\"><img src=\"" + this.imgDir + "chart_ignored.gif\" class=\"centeredImage\"></td>";
        }
        GP.View.write(this.displayId, inHTML + '</tr></table>');
    };
    ...
    this.drawBar = function () {
        ...
        for (var i = 0; i < this.success; i++) {
            inHTML = inHTML + "<td style=\"font-size: " + fontSize + "px; padding:0px 10px 0px 0px; font-weight:bold; font-family: Arial, sans-serif; text-align: center; white-space: nowrap; vertical-align:middle;background-color:#8CC06D;\">&nbsp;</td>";
        }
        for (var j = 0; j < this.failures; j++) {
            inHTML = inHTML + "<td style=\"font-size: " + fontSize + "px; padding:0px 10px 0px 0px; font-weight:bold; font-family: Arial, sans-serif; text-align: center; white-space: nowrap; vertical-align:middle;background-color:#BD0000;\">&nbsp;</td>";
        }
        for (var z = 0; z < limit; z++) {
            inHTML = inHTML + "<td style=\"font-size: " + fontSize + "px; padding:0px 10px 0px 0px; font-weight:bold; font-family: Arial, sans-serif; text-align: center; white-space: nowrap; vertical-align:middle;background-color:#D0D0D0;\">&nbsp;</td>";
        }
        GP.View.write(this.displayId, inHTML + '</tr></table>');
    };
